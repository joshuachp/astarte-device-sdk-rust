...
use crate::schema::{Aggregation, DatabaseRetentionPolicy, InterfaceType, Ownership};
...
impl Interface {
    ...
    /// Return a [`DatastreamIndividual`] if interface is [`Datastream`] and [`Individual`] .
    ///
    /// [`Datastream`]: InterfaceType::Datastream
    /// [`Individual`]: Aggregation::Individual
    #[must_use]
    pub fn into_datastream_individual(self) -> Option<DatastreamIndividual> {
        if let InterfaceTypeAggregation::DatastreamIndividual(v) = self.inner {
            Some(v)
        } else {
            None
    ...
    /// Return a [`DatastreamObject`] if interface is [`Datastream`] and [`Object`] .
    ///
    /// [`Datastream`]: InterfaceType::Datastream
    /// [`Object`]: Aggregation::Object
    #[must_use]
    pub fn into_datastream_object(self) -> Option<DatastreamObject> {
        if let InterfaceTypeAggregation::DatastreamObject(v) = self.inner {
            Some(v)
        } else {
            None
    ...
    /// Return a [`Properties`] if interface is [`Properties`] and [`Individual`] .
    ///
    /// [`Datastream`]: InterfaceType::Datastream
    /// [`Individual`]: Aggregation::Individual
    #[must_use]
...
impl Retention {
    ...
    pub fn as_expiry_seconds(&self) -> Option<i64> {
        let expiry = match self {
            Retention::Discard => return None,
            // Duration is copy
            Retention::Volatile { expiry } | Retention::Stored { expiry } => expiry,
        };
...
mod tests {
    ...
    use crate::{
        ...
        DatastreamIndividual, DatastreamIndividualMapping, Endpoint, Interface, Introspection,
        MappingPath,
    };
    ...
    fn test_iter_mappings() {
        ...
        let interface = Interface::from_str(INTERFACE_JSON)
            .unwrap()
            .into_datastream_individual()
            .unwrap();
    ...
    fn should_convert_into_inner() {
        ...
        let interface = interface.into_properties().unwrap();
