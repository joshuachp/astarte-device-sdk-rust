    ...
    T: AsRef<str> + Into<String>,
{
    ...
    fn try_from(value: Mapping<T>) -> Result<Self, Self::Error> {
        ...
        if value.reliability.is_some() {
            warn!("property cannot have reliability, ignoring");
        }
        ...
        if value.retention.is_some() {
            warn!("property cannot have retention, ignoring");
        }

        if value.expiry.is_some() {
            warn!("property cannot have expiry, ignoring");
        }

        if value.database_retention_policy.is_some() {
            warn!("property cannot have database_retention_policy, ignoring");
        }

        if value.database_retention_ttl.is_some() {
            warn!("property cannot have database_retention_ttl, ignoring");
        }
