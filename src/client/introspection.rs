...
where
    ...
    C::Sender: Register,
{
    async fn add_interface(&mut self, interface: Interface) -> Result<bool, NewError> {
        ...
        let map_err = interfaces
            .validate(interface)
            .wrap_err_with(|_| NewError::with(ErrorKind::Interface, "couldn't add interface"))?;
    ...
    async fn extend_interfaces<I>(&mut self, iter: I) -> Result<Vec<String>, NewError>
    where
        I: IntoIterator<Item = Interface> + Send,
    {
        ...
        let to_add = interfaces
            .validate_many(iter)
            .wrap_err_with(|_| NewError::with(ErrorKind::Interface, "couldn't add interfaces"))?;
    ...
    async fn add_interface_from_file<P>(&mut self, file_path: P) -> Result<bool, NewError>
    where
        P: AsRef<Path> + Send + Sync,
    {
        let interface = tokio::fs::read_to_string(&file_path)
            .await
            .wrap_err_with(|err| {
                NewError::with(ErrorKind::Io(err.kind()), "couldn't read interface")
                    .set_message(file_path.as_ref().display().to_string())
            })?;

        let interface = Interface::from_str(&interface).wrap_err_with(|_| {
            NewError::with(ErrorKind::Interface, "couldn't add interface")
                .set_message(file_path.as_ref().display().to_string())
        })?;
    ...
    async fn add_interface_from_str(&mut self, json_str: &str) -> Result<bool, NewError> {
        let interface = Interface::from_str(json_str)
            .wrap_err_ctx(ErrorKind::Interface, "couldn't add interface")?;

        self.add_interface(interface).await
    }

    async fn remove_interface(&mut self, interface_name: &str) -> Result<bool, NewError> {
        // Lock for writing for the whole scope, even the checks
        let mut interfaces = self.state.interfaces().write().await;
    ...
    async fn remove_interfaces<I>(&mut self, interfaces_name: I) -> Result<Vec<String>, NewError>
    where
