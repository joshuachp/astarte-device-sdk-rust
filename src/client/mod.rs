...
use crate::{
    ...
    transport::{grpc::GrpcError, mqtt::error::MqttError, Connection, Publish},
    Timestamp,
};
...
pub enum RecvError {
    ...
    /// Received a message wihtout timestamp, on an interface with `explicit_timestamp`
    #[error("message recieved with missing explicit timestamp on {interface_name}{path}")]
    MissingTimestamp {
...
impl RecvError {
    ...
    pub(crate) fn grpc_connection_error(value: GrpcError) -> Self {
        RecvError::Connection(value.into())
    }
}
