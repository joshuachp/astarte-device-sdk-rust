...
//!     .set_message(std::path::Path::new("/foo/bar").display())
...
pub trait WrapError<T, E>: Sized {
    ...
    fn wrap_err_ctx<K>(self, kind: K, ctx: &'static str) -> Result<T, Error<K>>
    where
        K: std::fmt::Display + std::fmt::Debug + PartialEq;
...
mod tests {
    ...
    fn should_wrap_err_ctx() {
        ...
        let Error {
            ...
            message,
            source,
        } = res.wrap_err_ctx(ErrorKind::Simple, exp_ctx).unwrap_err();
    ...
    fn should_wrap_err_with() {
        ...
        let Error {
            ...
            .wrap_err_with(|error| {
                Error::with(ErrorKind::Io(error.kind()), exp_ctx).set_message(exp_msg)
            })
            .unwrap_err();
